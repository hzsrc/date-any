...
function testFormat(date, format, result) {
    ...
    if (test !== result) console.error('Format fail: ' + test + '\t' + format + '\t != \t' + result)
}

function testCheck(format, dateStr) {
    var isMatch = new DateAny(format).check(dateStr)
    if (!isMatch) console.error(dateStr + '\t not match \t' + format)
}
...
function testParse(result, format, dateStr) {
    ...
    if (strRet !== strTest) console.error('Parse fail: ' + strTest + '\t' + format + '\t != \t' + strRet)
}
